...
class NHSOAuthClient:
    ...
    def login(self):
        try:
            ...
            # --- จุดที่ 1: เพิ่มเพื่อดูว่า Login ผ่านมั้ย ---
    ...
    def search_by_pid(self, pid):
        try:
            ...
            # --- จุดที่ 2: เพิ่มเพื่อดูว่า NHSO ส่งข้อมูลมามั้ย ---
            print(f">>> DEBUG NHSO: {resp.status_code} | {resp.text[:100]}")
...
def api_search():
    ...
    # --- จุดที่ 3: เพิ่มเพื่อดูว่า API TRUE ส่ง PID อะไรมา ---
    print(f">>> DEBUG SEARCH: MODE={mode} | QUERY={clean_query}")
